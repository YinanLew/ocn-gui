...
import { getAllWorkingEntries } from "@/lib/getAllWorkingEntries";
...
export default function WorkingHoursSubmitPage({
  ...
  useEffect(() => {
    async function fetchData() {
      if (status === "authenticated" && token && eventId) {
        try {
          const fetchedData = await getAllWorkingEntries(token, eventId);
  ...
  return (
    <div>
      <WorkingHoursTable apps={eventEntries} />
    </div>
  );
}
